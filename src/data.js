export const disciplinasBase = [
  {
    nome: "APH Tático",
    docente: "André Luís de Andrade / Monitor: Jeomar Moreira",
    cargahr: "10h",
    realizada: false,
    nota: false,
    avaliacao: false,
    // link: ""
  },
  {
    nome: "Análise de Cenários, Riscos e Gerenciamento de Crises",
    docente: "Cristiano Jacques",
    cargahr: "20h",
    realizada: false,
    nota: false,
    avaliacao: false,
    // link: ""
  },
  {
    nome: "Atividades Esportivas",
    docente: "Francisco Peixoto",
    cargahr: "30h",
    realizada: false,
    nota: false,
    avaliacao: false,
    // link: ""
  },
  {
    nome: "Comunicação Social",
    docente: "Luis Eduardo Meira",
    cargahr: "10h",
    realizada: true,
    nota: true,
    avaliacao: true,
    link: "https://forms.gle/Yf9BavhSt5Jur47BA"
  },
  {
    nome: "Condutores de Veículos de Emergência",
    docente: "Selton Sally", // conferir
    cargahr: "20h",
    realizada: false,
    nota: false,
    avaliacao: false,
    // link: ""
  },
  {
    nome: "Direção Defensiva e Evasiva",
    docente: "Jorge Luís Barreto / Monitor: Luciano", // conferir
    cargahr: "30h",
    realizada: false,
    nota: false,
    avaliacao: false,
    // link: ""
  },
  {
    nome: "Direitos Humanos - Abordagem Sóc. Psic. da violência e do Crime", // conferir nome exato
    docente: "Leonam Amitaf", // conferir
    cargahr: "5h",
    realizada: true,
    nota: true,
    avaliacao: false,
    // link: ""
  },
  {
    nome: "Direitos Humanos - Parte Geral",
    docente: "Daniella Vicuuna", // conferir
    cargahr: "10h",
    realizada: true,
    nota: false,
    avaliacao: true,
    link: "https://forms.gle/cePJQdvbQYbX3NWn8"
  },
  {
    nome: "Direitos Humanos - Polícia Comunitária",
    docente: "Hilmária Xavier",
    cargahr: "5h",
    realizada: true,
    nota: true,
    avaliacao: true,
    link: "https://forms.gle/Cm5epmAMVsy1Jycr7"
  },
  {
    nome: "Formalização de Procedimentos Policiais",
    docente: "Frankneyson Santos", // conferir
    cargahr: "60h",
    realizada: false,
    nota: false,
    avaliacao: false,
    // link: ""
  },
  {
    nome: "Fundamentos de Gestão Pública - Boas práticas de atendimento",
    docente: "Ivana Leite",
    cargahr: "10h",
    realizada: true,
    nota: true,
    avaliacao: true,
    link: "https://forms.gle/HF76tqSCviEQsBc2A"
  },
  {
    nome: "Fundamentos de Gestão Pública - Gestão de Pessoas e Liderança",
    docente: "Graciano Danillo", // conferir
    cargahr: "10h",
    realizada: true,
    nota: false,
    avaliacao: false,
    // link: ""
  },
  {
    nome: "Fundamentos de Gestão Pública - Gestão de Processos Administrativos",
    docente: "Lucas Natanael", // conferir
    cargahr: "5h",
    realizada: true,
    nota: true,
    avaliacao: true,
    link: "https://forms.gle/FqUz5BYm1MotDYGEA"
  },
  {
    nome: "Fundamentos de Gestão Pública -Sistema de Segurança Pública no Brasil e Gestão Integrada",
    docente: " Erilberto Antonio Maciel", // conferir
    cargahr: "5h",
    realizada: true,
    nota: true,
    avaliacao: true,
    link: "https://forms.gle/a81dWFbJfr64k9x66"
  },
  {
    nome: "Gestão da Informação - Análise Criminal, Estatística Aplicada à Segurança Pública e Geoprocessamento I",
    docente: "Cassandra Maria", // conferir
    cargahr: "5h",
    realizada: false,
    nota: false,
    avaliacao: false,
    // link: ""
  },
  {
    nome: "Gestão da Informação - Análise Criminal, Estatística Aplicada à Segurança Pública e Geoprocessamento II",
    docente: "Ellen Maria",
    cargahr: "5h",
    realizada: true,
    nota: false,
    avaliacao: true,
    link: "https://forms.gle/dkceLPSKKfrDJP7s7"
  },
  {
    nome: "Gestão da Informação - Análise Criminal, Estatística Aplicada à Segurança Pública e Geoprocessamento III",
    docente: "Glauber Welson", // conferir
    cargahr: "10h",
    realizada: true,
    nota: false,
    avaliacao: true,
    link: "https://forms.gle/tWWdpMiB6iwLwvfX7"
  },
  {
    nome: "Gestão da Informação - Sistema de Informação",
    docente: "Audemar Fernandes", // conferir
    cargahr: "5h",
    realizada: true,
    nota: true,
    avaliacao: true,
    link: "https://forms.gle/hHsHdxu2VqCybZwE7"
  },
  {
    nome: "Gestão da Informação - Sistemas da Polícia Civil",
    docente: "João Paulo B. / Monitor: Mª Elizabeth", // conferir
    cargahr: "15h",
    realizada: false,
    nota: false,
    avaliacao: false,
    // link: ""
  },
  {
    nome: "Introdução à Inteligência Policial",
    docente: "Rafaelle Narriman", // conferir
    cargahr: "20h",
    realizada: true,
    nota: false,
    avaliacao: true,
    link: "https://forms.gle/XeNeqdC1Qknvmt4z7"
  },
  {
    nome: "Investigação Policial I - Parte Geral",
    docente: "José Adriano Vieira",
    cargahr: "20h",
    realizada: true,
    nota: true,
    avaliacao: false,
    // link: ""
  },
  {
    nome: "Investigação Policial II - Crimes contra a Pessoa",
    docente: "Josenise de Andrade", // conferir
    cargahr: "20h",
    realizada: false,
    nota: false,
    avaliacao: false,
    // link: ""
  },
  {
    nome: "Investigação Policial II - Crimes contra a Pessoa - RIP",
    docente: "Diego de Farias",
    cargahr: "5h",
    realizada: false,
    nota: false,
    avaliacao: false,
    // link: ""
  },
  {
    nome: "Investigação Policial III - Crimes de Feminicídio - Módulo I",
    docente: "Jaíne Araújo", // conferir
    cargahr: "5h",
    realizada: true,
    nota: true,
    avaliacao: true,
    link: "https://forms.gle/si1K1y3RfnZeFRoJ8"
  },
  {
    nome: "Investigação Policial III - Crimes de Feminicídio - Módulo II",
    docente: "Maira Roberta",
    cargahr: "10h",
    realizada: false,
    nota: false,
    avaliacao: false,
    // link: ""
  },
  {
    nome: "Investigação Policial IV - Crimes de Tráfico de Entorpecentes",
    docente: "Thiago de Vasconcelos",
    cargahr: "20h",
    realizada: true,
    nota: false,
    avaliacao: true,
    link: "https://forms.gle/79XeDd4TsaifKSZMA"
  },
  {
    nome: "Investigação Policial IX - Crimes contra vulneráveis: crimes contra a mulher",
    docente: "Maria Sileide / Monitor: Waleska Gabínio", // conferir
    cargahr: "15h",
    realizada: false,
    nota: false,
    avaliacao: false,
    // link: ""
  },
  {
    nome: "Investigação Policial IX - Crimes contra vulneráveis: crimes contra o consumidor",
    docente: "Paulo Ênio",
    cargahr: "5h",
    realizada: true,
    nota: false,
    avaliacao: false,
    // link: ""
  },
  {
    nome: "Investigação Policial IX - Crimes contra vulneráveis: crimes contra o público LGBTQIA+",
    docente: "Marcelo Antas", // conferir
    cargahr: "5h",
    realizada: false,
    nota: false,
    avaliacao: false,
    // link: ""
  },
  {
    nome: "Investigação Policial IX - Crimes contra vulneráveis: crimes contra pessoa idosa",
    docente: "Pedro Ivo",
    cargahr: "10h",
    realizada: false,
    nota: false,
    avaliacao: false,
    // link: ""
  },  
  {
    nome: "Investigação Policial IX - Crimes contra vulneráveis - Crimes de Preconceito",
    docente: "Hilmária",
    cargahr:"20h",
    realizada: false,
    nota: false,
    avaliacao: false,
    // link: ""
  },
  {
    nome: "Investigação Policial V - Crimes Cibernéticos - Busca Eletrônica",
    docente: "Jeorgy Vennâncio/ Monitor: João Ildefonso",
    cargahr:"20h", 
    realizada: false,
    nota: false,
    avaliacao: false,
    // link: ""
  },
  {
    nome: "Investigação Policial V - Crimes Cibernéticos/ Busca Eletrônica - Módulo Crimes Cibernéticos I",
    docente: "José Wanderlúcio",
    cargahr:"5h",
    realizada: false,
    nota: false,
    avaliacao: false,
    // link: ""
  },
    {
    nome: "Investigação Policial V - Crimes Cibernéticos / Busca Eletrônica - Módulo Crimes Cibernéticos II",
    docente: "João Ricardo Monteiro",
    cargahr:"5h",
    realizada: false,
    nota: false,
    avaliacao: false,
    // link: ""
  },
    {
    nome: "Investigação Policial V - Crimes Cibernéticos / Busca Eletrônica - Módulo Crimes Cibernéticos III",
    docente: "Sandro Mauro Rezende",
    cargahr:"5h",
    realizada: false,
    nota: false,
    avaliacao: false,
    // link: ""
  },
    {
    nome: "Investigação Policial V - Crimes Cibernéticos / Busca Eletrônica - Módulo Crimes Cibernéticos IV",
    docente: "Mauricio Wanderley",
    cargahr:"5h",
    realizada: false,
    nota: false,
    avaliacao: false,
    // link: ""
  },
    {
    nome: "Investigação Policial VI - Crimes contra o Patrimônio: Roubo à residência e transeunte",
    docente: "Elizabeth Regina",
    cargahr:"5h",
    realizada: true,
    nota: true,
    avaliacao: true,
    link: "https://forms.gle/X3qcn912Mdugd78t8"
  },
    {
    nome: "Investigação Policial VI - Crimes contra o Patrimônio: Roubo a veículos e cargas",
    docente: "Lucas Rothardand",
    cargahr:"15h",
    realizada: true,
    nota: true,
    avaliacao: false,
    // link: ""
  },
    {
    nome: "Investigação Policial VI - Crimes Contra o Patrimônio, Roubo a instituições financeiras",
    docente: "Fábio Vinicius",
    cargahr:"10h",
    realizada: false,
    nota: false,
    avaliacao: false,
    // link: ""
  },
    {
    nome: "Investigação Policial VII - Crimes Contra Lavagem de dinheiro e combate ao crime Organizado - Lavagem de Dinheiro",
    docente: "Eduardo de Almeida",
    cargahr:"10h",
    realizada: false,
    nota: false,
    avaliacao: false,
    // link: ""
  },
    {
    nome: "Investigação Policial VII - Crimes Contra Lavagem de dinheiro e combate ao crime Organizado - Crime Organizado",
    docente: "Claúdio Romero",
    cargahr:"10h",
    realizada: false,
    nota: false,
    avaliacao: false,
    // link: ""
  },
  {
    nome: "Investigação Policial VIII - Crimes de estelionato e outras fraudes",
    docente: "Luiz Cerqueira Cotrim Neto", // conferir
    cargahr: "10h",
    realizada: true,
    nota: false,
    avaliacao: true,
    link: "https://forms.gle/Dggbke2CQUTRbGcY7"
  },
  {
    nome: "Investigação Policial X - Crimes contra a infância e a juventude / Escuta qualificada",
    docente: "Andrea Melo", // conferir
    cargahr: "10h",
    realizada: false,
    nota: false,
    avaliacao: false,
    // link: ""
  },
  {
    nome: "Investigação Policial X - Crimes contra a infância e a juventude / Escuta qualificada",
    docente: "Adriana Guedes",
    cargahr: "10h",
    realizada: false,
    nota: false,
    avaliacao: false,
    // link: ""
  },
  {
    nome: "Investigação Policial XI - Legislação Criminal Especial - Crimes Ambientais",
    docente: "Ellen Maria",
    cargahr: "5h",
    realizada: false,
    nota: false,
    avaliacao: false,
    // link: ""
  },
  {
    nome: "Investigação Policial XI - Legislação Criminal Especial - Crimes de Menor Potencial",
    docente: "Paulo Ênio",
    cargahr: "5h",
    realizada: false,
    nota: false,
    avaliacao: false,
    // link: ""
  },
  {
    nome: "Investigação Policial XI - Legislação Criminal Especial - Interceptação telefônica e dados",
    docente: "Thiago de Vasconcelos",
    cargahr: "5h",
    realizada: false,
    nota: false,
    avaliacao: false,
    // link: ""
  },
  {
    nome: "Investigação Policial XI - Legislação Criminal Especial - Prisões cautelares",
    docente: "Francisco Jarley",
    cargahr: "5h",
    realizada: false,
    nota: false,
    avaliacao: false,
    // link: ""
  },
  {
    nome: "Isolamento, preservação do local do crime, valorização da prova e cadeia de custódia - Módulo I",
    docente: "Gilson Duarte",
    cargahr: "15h",
    realizada: false,
    nota: false,
    avaliacao: false,
    // link: ""
  },
  {
    nome: "Isolamento, preservação do local do crime, valorização da prova e cadeia de custódia - Módulo II",
    docente: "Gabriella Henrique",
    cargahr: "5h",
    realizada: false,
    nota: false,
    avaliacao: false,
    // link: ""
  },
  {
    nome: "Perícias Legais - Balística",
    docente: "Vanduir Soares",
    cargahr: "5h",
    realizada: true,
    nota: false,
    avaliacao: true,
    link: "https://forms.gle/h1EuKAdDncpLe6kt7"
  },
  {
    nome: "Perícias Legais - Criminalística",
    docente: "Adriano José", // conferir
    cargahr: "15h",
    realizada: false,
    nota: false,
    avaliacao: false,
  },
  {
    nome: "Perícias Legais - Informática, imagem e som",
    docente: "Ana Carla Cardoso",
    cargahr: "10h",
    realizada: false,
    nota: false,
    avaliacao: false,
  },
  {
    nome: "Perícias Legais - Laboratórios",
    docente: "Marcelo Barbosa",
    cargahr: "10h",
    realizada: false,
    nota: false,
    avaliacao: false,
  },
  {
    nome: "Perícias Legais - Medicina Legal",
    docente: "Eronildo Ribeiro", // conferir
    cargahr: "10h",
    realizada: true,
    nota: true,
    avaliacao: true,
    link: "https://forms.gle/RgPZGn59UKit5LEXA"
  },
  {
    nome: "Planejamento Operacional",
    docente: "Clodoaldo S.",
    cargahr: "20h",
    realizada: false,
    nota: false,
    avaliacao: false,
  },
  {
    nome: "Rádio e Comunicação Policial",
    docente: "Fabiano de Abrantes",
    cargahr: "10h",
    realizada: true,
    nota: true,
    avaliacao: true,
    link: "https://forms.gle/z7vEripbFiXKtvYZ7"
  },
  {
    nome: "Recuperação de Ativos",
    docente: "Carlos Othon", // conferir
    cargahr: "5h",
    realizada: false,
    nota: false,
    avaliacao: false,
  },
  {
    nome: "Técnicas de Entrevista e Interrogatório",
    docente: "Alluska Cristiniee", // conferir
    cargahr: "20h",
    realizada: false,
    nota: false,
    avaliacao: false,
  },
  {
    nome: "Uso Legal e Prog. da Força: Defesa Pessoal e Instr. Tático",
    docente: "Leonardo Dantas",
    cargahr: "25h",
    realizada: false,
    nota: false,
    avaliacao: false,
  },
  {
    nome: "Uso Legal e Prog. da Força: Tática e Abordagem Policial",
    docente: "Paulo Nogueira/ Monitor: Jeomar Moreira e Samuel Fernando e Isa",
    cargahr: "40h",
    realizada: false,
    nota: false,
    avaliacao: false,
  },
  {
    nome: "Uso Legal e Prog. da Força: Tiro Defensivo",
    docente: "Thiago Fagner/ Monitor: Evaldo Tavares, Simone de Almeida e Wagner Ubiratan",
    cargahr: "60h",
    realizada: false,
    nota: false,
    avaliacao: false,
  }   
];